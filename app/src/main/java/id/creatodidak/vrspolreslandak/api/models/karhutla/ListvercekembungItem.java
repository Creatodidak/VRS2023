package id.creatodidak.vrspolreslandak.api.models.karhutla;

import com.google.gson.annotations.SerializedName;

import java.io.Serializable;

public class ListvercekembungItem implements Serializable {

	@SerializedName("verifikasi")
	private String verifikasi;

	@SerializedName("jabatanpers")
	private String jabatanpers;

	@SerializedName("pelaksana")
	private String pelaksana;

	@SerializedName("latitude")
	private Object latitude;

	@SerializedName("xkecamatan")
	private String xkecamatan;

	@SerializedName("created_at")
	private String createdAt;

	@SerializedName("geocoder")
	private String geocoder;

	@SerializedName("prediksi")
	private String prediksi;

	@SerializedName("pangkatpers")
	private String pangkatpers;

	@SerializedName("xkode")
	private String xkode;

	@SerializedName("personil")
	private String personil;

	@SerializedName("updated_at")
	private String updatedAt;

	@SerializedName("volumeair")
	private String volumeair;

	@SerializedName("rekomendasi")
	private String rekomendasi;

	@SerializedName("id")
	private int id;

	@SerializedName("xlatitude")
	private Object xlatitude;

	@SerializedName("longitude")
	private Object longitude;

	@SerializedName("xtype")
	private String xtype;

	@SerializedName("cuaca")
	private String cuaca;

	@SerializedName("analisa")
	private String analisa;

	@SerializedName("kodeembung")
	private String kodeembung;

	@SerializedName("xlongitude")
	private Object xlongitude;

	@SerializedName("satkerpers")
	private String satkerpers;

	@SerializedName("xkapasitas")
	private String xkapasitas;

	@SerializedName("xnama")
	private String xnama;

	@SerializedName("dataid")
	private String dataid;

	@SerializedName("foto")
	private String foto;

	@SerializedName("namapers")
	private String namapers;

	@SerializedName("fotopers")
	private String fotopers;

	@SerializedName("nrppers")
	private String nrppers;

	@SerializedName("xfoto")
	private String xfoto;

	@SerializedName("intensitasair")
	private String intensitasair;

	@SerializedName("satfungpers")
	private String satfungpers;

	public void setVerifikasi(String verifikasi){
		this.verifikasi = verifikasi;
	}

	public String getVerifikasi(){
		return verifikasi;
	}

	public void setJabatanpers(String jabatanpers){
		this.jabatanpers = jabatanpers;
	}

	public String getJabatanpers(){
		return jabatanpers;
	}

	public void setPelaksana(String pelaksana){
		this.pelaksana = pelaksana;
	}

	public String getPelaksana(){
		return pelaksana;
	}

	public void setLatitude(Object latitude){
		this.latitude = latitude;
	}

	public Object getLatitude(){
		return latitude;
	}

	public void setXkecamatan(String xkecamatan){
		this.xkecamatan = xkecamatan;
	}

	public String getXkecamatan(){
		return xkecamatan;
	}

	public void setCreatedAt(String createdAt){
		this.createdAt = createdAt;
	}

	public String getCreatedAt(){
		return createdAt;
	}

	public void setGeocoder(String geocoder){
		this.geocoder = geocoder;
	}

	public String getGeocoder(){
		return geocoder;
	}

	public void setPrediksi(String prediksi){
		this.prediksi = prediksi;
	}

	public String getPrediksi(){
		return prediksi;
	}

	public void setPangkatpers(String pangkatpers){
		this.pangkatpers = pangkatpers;
	}

	public String getPangkatpers(){
		return pangkatpers;
	}

	public void setXkode(String xkode){
		this.xkode = xkode;
	}

	public String getXkode(){
		return xkode;
	}

	public void setPersonil(String personil){
		this.personil = personil;
	}

	public String getPersonil(){
		return personil;
	}

	public void setUpdatedAt(String updatedAt){
		this.updatedAt = updatedAt;
	}

	public String getUpdatedAt(){
		return updatedAt;
	}

	public void setVolumeair(String volumeair){
		this.volumeair = volumeair;
	}

	public String getVolumeair(){
		return volumeair;
	}

	public void setRekomendasi(String rekomendasi){
		this.rekomendasi = rekomendasi;
	}

	public String getRekomendasi(){
		return rekomendasi;
	}

	public void setId(int id){
		this.id = id;
	}

	public int getId(){
		return id;
	}

	public void setXlatitude(Object xlatitude){
		this.xlatitude = xlatitude;
	}

	public Object getXlatitude(){
		return xlatitude;
	}

	public void setLongitude(Object longitude){
		this.longitude = longitude;
	}

	public Object getLongitude(){
		return longitude;
	}

	public void setXtype(String xtype){
		this.xtype = xtype;
	}

	public String getXtype(){
		return xtype;
	}

	public void setCuaca(String cuaca){
		this.cuaca = cuaca;
	}

	public String getCuaca(){
		return cuaca;
	}

	public void setAnalisa(String analisa){
		this.analisa = analisa;
	}

	public String getAnalisa(){
		return analisa;
	}

	public void setKodeembung(String kodeembung){
		this.kodeembung = kodeembung;
	}

	public String getKodeembung(){
		return kodeembung;
	}

	public void setXlongitude(Object xlongitude){
		this.xlongitude = xlongitude;
	}

	public Object getXlongitude(){
		return xlongitude;
	}

	public void setSatkerpers(String satkerpers){
		this.satkerpers = satkerpers;
	}

	public String getSatkerpers(){
		return satkerpers;
	}

	public void setXkapasitas(String xkapasitas){
		this.xkapasitas = xkapasitas;
	}

	public String getXkapasitas(){
		return xkapasitas;
	}

	public void setXnama(String xnama){
		this.xnama = xnama;
	}

	public String getXnama(){
		return xnama;
	}

	public void setDataid(String dataid){
		this.dataid = dataid;
	}

	public String getDataid(){
		return dataid;
	}

	public void setFoto(String foto){
		this.foto = foto;
	}

	public String getFoto(){
		return foto;
	}

	public void setNamapers(String namapers){
		this.namapers = namapers;
	}

	public String getNamapers(){
		return namapers;
	}

	public void setFotopers(String fotopers){
		this.fotopers = fotopers;
	}

	public String getFotopers(){
		return fotopers;
	}

	public void setNrppers(String nrppers){
		this.nrppers = nrppers;
	}

	public String getNrppers(){
		return nrppers;
	}

	public void setXfoto(String xfoto){
		this.xfoto = xfoto;
	}

	public String getXfoto(){
		return xfoto;
	}

	public void setIntensitasair(String intensitasair){
		this.intensitasair = intensitasair;
	}

	public String getIntensitasair(){
		return intensitasair;
	}

	public void setSatfungpers(String satfungpers){
		this.satfungpers = satfungpers;
	}

	public String getSatfungpers(){
		return satfungpers;
	}
}