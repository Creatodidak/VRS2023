...
public class CekHotspotVerifikator extends AppCompatActivity {
    Endpoint endpoint;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_cek_hotspot_verifikator);
        ...
        adapter = new ListVerCekHotspotAdp(this, list, new ListVerCekHotspotAdp.OnItemClickListener() {
            ...
            public void onClick(ListvercekhotspotItem data) {
                Intent intent = new Intent(CekHotspotVerifikator.this, ShowCekHotspotDataVerifikator.class);
        ...
        icCalendar.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                new CustomDatePickerDialog(CekHotspotVerifikator.this, new CustomDatePickerDialog.OnDateSetListener() {
                    @Override
    ...
    private void fetchlist(List<ListvercekhotspotItem> listVerCekHotspot) {
        ...
        if (listVerCekHotspot.size() != 0) {
            for (ListvercekhotspotItem item : listVerCekHotspot) {
                if (jabatan.contains("KABAG")) {
                    if (!item.getVerifikasi().equals("BELUM ADA") && !item.getVerifikasi().contains("POLSEK") && !item.getVerifikasi().equals("POLSUB")) {
                        list.add(item);
                    }
                } else if (jabatan.contains("KAPOLSEK")) {
                    if (!item.getVerifikasi().equals("BELUM ADA") || !item.getVerifikasi().equals("POLSUB")) {
                        list.add(item);
                    }
                } else if (jabatan.contains("KAPOLSUBSEKTOR")) {
                    if (!item.getVerifikasi().equals("BELUM ADA") || !item.getVerifikasi().equals("POLSEK")) {
                        list.add(item);
                    }
                }
            }
